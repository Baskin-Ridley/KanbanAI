  ...
  useEffect(() => {
    ...
    if (submitted && password) {
      const change_pass = async () => {
        ...
        try {
          const response = await fetch(
            "https://built-differently-backend.onrender.com/user/password",
            options
          );
          const listen = await response.json();
          console.log(listen);
    ...
    if (submitted && email) {
      const change_pass = async () => {
        ...
        try {
          const response = await fetch(
            "https://built-differently-backend.onrender.com/user/email",
            options
          );
          const listen = await response.json();
          console.log(listen);
  ...
  return (
    <>
      <main className="flex flex-col items-center justify-center mx-auto">
        ...
        <form
          className="max-w-md m-2 w-64 rounded-lg border border-gray-400 bg-blue-50 px-2 py-3 "
        ...
        >
          <div className="mb-4">
            <Form_Input
              ...
              ariaLabel="Field for inputting the password"
            />
          </div>
          <div className="mb-4">
            <Form_Input
              ...
              formElementId="email"
              ariaLabel="Field for inputting the email"
            />
          </div>
          {user.isSuper && (
            <div className="mb-4">
              <Form_Input
                ...
                formElementId="add members"
                ariaLabel="Field for inputting the members"
              />
            </div>
          )}
          <Form_Button
            buttonText="Update"
            ariaLabel="Button for submitting the form"
          />
        </form>
      </main>
    </>
