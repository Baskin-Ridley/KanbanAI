...
const AdminChart = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      const response = await fetch(
        `https://built-differently-backend.onrender.com/users/${user.username}/kanban_boards`
      );
  ...
  const handleTasks = async (id) => {
    ...
    let response = await fetch(
      `https://built-differently-backend.onrender.com/kanban-boards/${id}/tickets`
    );
  ...
  const handleNameClick = async (user_id) => {
    console.log(user_id);
    const response = await fetch(
      `https://built-differently-backend.onrender.com/users/${user_id}/kanban_boards`
    );
    const data = await response.json();
    console.log(data);
    setGanttBoard(data);
                ...
                {userBoard &&
                  userBoard.map((e, i) => (
                    <>
                      {" "}
                      <button
                        class="wrap border flex  justify-center mx-auto m-2 rounded-lg border-gray-400 bg-blue-50 mb-2 rounded-md bg-blue-50 py-2 px-4 text-sm text-center shadow-md border-gray-400 transition-colors duration-150 hover:bg-blue-300 hover:text-white false w-full"
                        onClick={() => handleNameClick(e.user_id)}
                      >
                        <li>user: {e.user_name}</li>
                      </button>
                    </>
                  ))}
                <br />
