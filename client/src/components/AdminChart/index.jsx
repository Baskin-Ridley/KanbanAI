...
const AdminChart = () => {
  ...
  useEffect(() => {
    const fetchAllKanban = async () => {
      ...
      for (let item of userBoard) {
        const response = await fetch(
          `https://built-differently-backend.onrender.com/users/${item.id}/kanban_boards`
        );
        const data = await response.json();
        console.log(data);
  ...
  //     let ticketsArray = []
  //     let response = await fetch(`https://built-differently-backend.onrender.com/kanban-boards/${id}/tickets`);
